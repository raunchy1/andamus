...
export function SardiniaMap({ className, mode = "desktop", onRouteClick, onCityClick }: SardiniaMapProps) {
  ...
  // Fix #7: The tooltip uses fixed positioning calculated from
  // getBoundingClientRect(), which is only available in the browser. SSR
  // renders position {0, 0} while the client renders the real coordinates,
  // causing a React hydration mismatch. We suppress the tooltip entirely until
  // after the first client render by tracking a `mounted` flag.
  ...
  return (
    <div className={`bg-[#0a0a0a] rounded-2xl overflow-hidden border border-white/5 ${className || ""}`}>
          ...
          {cities.map((city) => (
            <g
              ...
              onMouseEnter={(e) => {
                ...
                // Fix #7: only read getBoundingClientRect after mount (client only)
                if (mounted) {
      ...
      {/* Fix #7: Tooltip is only rendered after mount so SSR and the initial
          client render agree on the DOM (no tooltip), avoiding a hydration
          mismatch from the getBoundingClientRect-derived position. */}
      {mounted && hoveredCity && (
        <div
