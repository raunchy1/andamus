  ...
  return (
    <div ref={dropdownRef} className={`relative ${className}`}>
      ...
      <button
        ...
        className="w-full flex items-center justify-between gap-2 px-4 py-3.5 rounded-2xl bg-elevated border border-transparent focus-within:border-primary/50 text-left transition-all"
      >
        <div className="flex items-center gap-2 min-w-0">
          <Search className="w-5 h-5 text-fg/30 shrink-0" />
          {value ? (
            <span className="font-semibold text-fg truncate">{value.name}</span>
          ) : (
        ...
        <div className="flex items-center gap-1 shrink-0">
          {value && (
            <span
              role="button"
              onClick={handleClear}
              className="p-1 rounded-full hover:bg-elevated text-fg/40 hover:text-fg transition-all"
            >
              <X className="w-4 h-4" />
            </span>
          )}
          <ChevronDown
            className={`w-4 h-4 text-fg/40 transition-transform ${open ? "rotate-180" : ""}`}
          />
        </div>
      </button>
      ...
      {/* Dropdown */}
      {open && (
        <div className="absolute top-full left-0 w-full mt-2 z-50 bg-surface border border-line/30 rounded-2xl shadow-2xl overflow-hidden">
          {/* Search input */}
          <div className="p-3 border-b border-line/20">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-elevated rounded-xl py-2.5 pl-10 pr-4 text-sm text-fg placeholder:text-fg/30 border-none focus:outline-none"
              />
            </div>
          </div>
          ...
          <div className="max-h-[280px] overflow-y-auto">
            {loading ? (
              <div className="flex items-center justify-center py-8">
                <Loader2 className="w-5 h-5 animate-spin text-fg/40" />
              </div>
            ) : makes.length === 0 ? (
...
function MakeOption({
  ...
  return (
    <button
      ...
      className={`w-full flex items-center justify-between px-4 py-2.5 text-sm transition-all hover:bg-elevated ${
        isSelected ? "text-primary bg-primary/5" : "text-fg"
      }`}
    >
      <span className="font-medium">{make.name}</span>
      <div className="flex items-center gap-2">
        {make.country && (
          <span className="text-[10px] text-fg/40 font-bold">{make.country}</span>
        )}
    ...
    <div ref={dropdownRef} className={`relative ${className}`}>
      <button
        ...
        className={`w-full flex items-center justify-between gap-2 px-4 py-3.5 rounded-2xl border border-transparent text-left transition-all ${
          disabled
            ? "bg-elevated/50 cursor-not-allowed opacity-50"
            : "bg-elevated hover:border-primary/30 focus-within:border-primary/50"
        }`}
      >
        <div className="flex items-center gap-2 min-w-0">
          <Search className="w-5 h-5 text-fg/30 shrink-0" />
        ...
        <ChevronDown
          className={`w-4 h-4 text-fg/40 shrink-0 transition-transform ${open ? "rotate-180" : ""}`}
        />
      </button>
              ...
              <div className="py-2">
                {models.map((model) => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-4 py-2.5 text-sm transition-all hover:bg-elevated ${
                      value?.id === model.id ? "text-primary bg-primary/5" : "text-fg"
                    }`}
                  >
                    <span className="font-medium">{model.name}</span>
                    <div className="flex items-center gap-2">
                      {model.body_type && (
                        <span className="text-[10px] text-fg/40 capitalize">
                          {model.body_type}
                        </span>
                      )}
