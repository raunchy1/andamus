    ...
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4">
      <div className="w-full sm:w-full sm:max-w-md bg-surface sm:rounded-3xl rounded-t-3xl border border-line shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-3">
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-full bg-surface flex items-center justify-center text-muted hover:text-ink hover:bg-sand-deep transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          {sent ? (
            ...
            <>
              ...
              <textarea
                ...
                rows={4}
                maxLength={1000}
                className="w-full bg-surface border border-line rounded-xl px-4 py-3 text-sm text-ink placeholder:text-faint focus:outline-none focus:border-green/50 resize-none"
              />
