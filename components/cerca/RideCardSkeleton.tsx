...
function SkeletonLine({ className }: { className?: string }) {
  return (
    <div
      className={`rounded-md bg-surface shimmer ${className || ""}`}
    />
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="relative py-6 sm:py-8 flex items-center justify-between">
        <div className="absolute left-0 right-0 h-[2px] rounded bg-surface" />
        ...
        <div className="relative z-10 flex flex-col items-center px-2 sm:px-4 flex-shrink-0">
          <div className="w-5 h-5 sm:w-6 sm:h-6 rounded bg-surface" />
        </div>
      ...
      {/* Driver Info */}
      <div className="flex items-center justify-between mt-4 sm:mt-6 pt-4 border-t border-line">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-surface flex-shrink-0" />
