...
export async function getMarketplaceSignals(): Promise<MarketplaceSignals> {
  ...
  const { count: ridesToday, error: ridesError } = await supabase
    .from("rides")
  ...
  let simulatedActiveCount = 12; // base level of mock active commuters
  ...
  const result: MarketplaceSignals = {
    ridesAddedToday: ridesToday || 0,
    activeCommutersCount: simulatedActiveCount,
    trendingRoute,
    bookingsCompletedToday: bookingsToday || 0,
