...
export async function POST(request: NextRequest) {
  try {
    ...
    if (bookingError || !booking) {
      console.error("[booking-rejected API] Booking not found:", bookingError?.message);
    ...
    if (bookingData.ride.driver.id !== user.id) {
      return NextResponse.json(
        { error: "Forbidden: Only the driver of this ride can reject bookings and send notification" },
        { status: 403 }
      );
    }
    ...
    if (passengerUserError || !passengerUser?.user?.email) {
      console.error("[booking-rejected API] Passenger auth account not found:", passengerUserError?.message);
    ...
    if (passengerError || !passengerProfile) {
      console.error("[booking-rejected API] Passenger profile not found:", passengerError?.message);
      return NextResponse.json({ error: "Passenger profile not found" }, { status: 404 });
    }

    // 6. Send the booking rejected email using verified database data (H-02)
